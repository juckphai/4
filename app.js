    ...
    const App = {
        ...
        toggleSection(sectionId) {
            ...
            if (currentlyOpenSection) {
                ...
                const currentlyOpenHeader = document.querySelector('.section-header.active');
        ...
        handleSummaryOutput(choice) {
            ...
            if (this.summaryContext.type === 'detailed_list') {
                 if (choice === 'display') {
                    const html = this.buildDetailedListHtml(this.summaryContext);
                    this.openSummaryModal(html);
                } else if (choice === 'csv') {
                    this.exportDetailedListToCsv(this.summaryContext);
                }
        ...
        _runSummary(startDate, endDate, title, periodName, sellerId = null) { 
            ...
            this.summaryContext = { type: 'aggregated_pos', summaryResult, title, dateString, thaiDateString, periodName, sellerIdFilter: sellerId, startDate, endDate }; 
            this.openSummaryOutputModal(); 
        },
        ...
        exportDetailedListToCsv(context) {
            ...
            csvRows.push([title]);
            csvRows.push(['ช่วงวันที่:', thaiDateString]);
            csvRows.push(['สรุปเมื่อ:', summaryTimestamp]);
            csvRows.push([]); 
            ...
            if (isSellerReport && user.commissionRate > 0) {
                ...
                if(commissionDetails.length > 0) {
                    ...
                    commissionDetails.forEach(detail => {
                         csvRows.push(['', '', '', '', '', `${detail.label}: ${this.formatNumberSmart(detail.amount)} บาท`, `ค่าคอมฯ: ${this.formatNumberSmart(detail.commission)} บาท`]);
                    });
            ...
            const fileName = `${periodName}_${new Date().getTime()}.csv`;
        ...
        async saveBackupToFile() {
            ...
            const fullFileName = `pos_backup_${currentUser}_${dateTimeString}.json`;
        ...
        getPageContent(pageId) {
            const templates = {
                ...
                'page-users': `<h2>จัดการผู้ใช้</h2> <form id="user-form"> <input type="hidden" id="user-id"> <div class="form-group"><label for="user-username">ชื่อผู้ใช้:</label><input type="text" id="user-username" required></div><div class="form-group"><label for="user-password">รหัสผ่านใหม่:</label><input type="password" id="user-password" placeholder="เว้นว่างไว้ถ้าไม่ต้องการเปลี่ยน"></div><div class="form-group"><label for="user-password-confirm">ยืนยันรหัสผ่าน:</label><input type="password" id="user-password-confirm" placeholder="เว้นว่างไว้ถ้าไม่ต้องการเปลี่ยน"></div><div class="form-group"><label style="font-weight:normal; display:block; cursor:pointer;"><input type="checkbox" id="show-password-user-form"> แสดงรหัสผ่าน</label></div><div class="form-group"><label for="user-role">ประเภท:</label><select id="user-role" required> <option value="seller">Seller</option> <option value="admin">Admin</option> </select></div><div id="user-store-assignment-container" class="form-group"></div><div id="user-commission-settings-container" class="form-group"> <h4>ตั้งค่าคอมมิชชั่น</h4><div class="form-group"><label for="user-commission-rate">อัตรา (%):</label><input type="number" id="user-commission-rate" min="0" max="100" step="any" placeholder="เช่น 3, 5.5"></div><div class="form-group"><label>คิดจากยอดขาย:</label><div id="user-commission-sources"> <label><input type="checkbox" id="user-commission-cash"> เงินสด</label> <label><input type="checkbox" id="user-commission-transfer"> เงินโอน</label> <label><input type="checkbox" id="user-commission-credit"> เครดิต</label> </div></div></div> <div id="user-history-view-container" class="form-group"> <h4>ตั้งค่าการแสดงผลประวัติ</h4><div class="form-group"><label for="user-visible-days">จำนวนวันที่ดูประวัติขายได้:</label><input type="number" id="user-visible-days" min="0" placeholder="0=วันนี้, 1=วันนี้และเมื่อวาน (เว้นว่าง=ทั้งหมด)"></div></div> <div id="user-sales-period-container" class="form-group"><h4>กำหนดระยะเวลาที่สามารถขายได้</h4> <div class="form-group"><label for="user-sales-start-date">วันที่เริ่มขาย:</label><input type="date" id="user-sales-start-date"></div><div class="form-group"><label for="user-sales-end-date">วันที่สิ้นสุด:</label><input type="date" id="user-sales-end-date"></div></div> <div id="user-product-assignment-container" class="form-group"> <h4>กำหนดสินค้าที่สามารถขายได้</h4> <div id="user-product-assignment" style="max-height: 200px; overflow-y: auto; border: 1px solid #BFBFBF; padding: 10px; border-radius: 10px;"></div> </div> <div class="form-actions"> <button type="submit" class="success">บันทึกผู้ใช้</button> <button type="button" id="clear-user-form-btn" style="background-color:#6c757d;">เคลียร์ฟอร์ม</button> </div> </form> <div class="table-container"><table id="user-table"> <thead> <tr><th>ชื่อผู้ใช้</th><th>ประเภท</th><th>ร้านค้า</th><th>สินค้าที่ขายได้</th><th>ระยะเวลาที่ขายได้</th><th>จัดการ</th></tr> </thead> <tbody></tbody> </table></div>`,
                'page-data': `<h2>จัดการข้อมูล</h2><div class="data-management-section admin-only data-restore-section"><h3>โหลดข้อมูลจากไฟล์ (Restore)</h3><p style="color: var(--danger-color);"><b>คำเตือน:</b> การโหลดข้อมูลจากไฟล์จะรวมข้อมูลเข้ากับข้อมูลปัจจุบัน ข้อมูลที่ซ้ำกันจะถูกเขียนทับด้วยข้อมูลจากไฟล์!</p><input type="file" id="data-file-input" style="display: none;" accept=".json,application/json"><button type="button" id="load-from-file-btn" style="background-color: #E97132;">เลือกไฟล์สำรอง (.json)</button></div><div class="data-management-section admin-only"><h3>ตั้งรหัสผ่านสำหรับไฟล์สำรอง</h3><p>รหัสผ่านนี้จะใช้เข้ารหัสไฟล์สำรองข้อมูลที่สร้างโดยแอดมินโดยอัตโนมัติ</p><form id="backup-password-form" style="max-width: 400px;"><div class="form-group"><label for="backup-password">รหัสผ่านใหม่ (เว้นว่างเพื่อลบ):</label><input type="password" id="backup-password" placeholder="พิมพ์รหัสผ่านที่นี่"></div><div class="form-group"><label for="backup-password-confirm">ยืนยันรหัสผ่านใหม่:</label><input type="password" id="backup-password-confirm" placeholder="พิมพ์รหัสผ่านอีกครั้ง"></div><div class="form-group"><label style="font-weight: normal; cursor: pointer;"><input type="checkbox" id="show-backup-password"> แสดงรหัสผ่าน</label></div><div class="form-actions" style="justify-content: center;"><button type="submit" class="success">บันทึกรหัสผ่าน</button></div></form><p id="password-status" style="font-weight: bold; margin-top: 15px;"></p></div><div class="data-management-section admin-only"><h3>สำรองข้อมูล (Backup)</h3><p>สำรองข้อมูลทั้งหมด (ผู้ใช้, สินค้า, ประวัติการขาย) ลงในไฟล์ JSON เพื่อเก็บไว้หรือย้ายไปยังเครื่องอื่น</p><button id="save-to-file-btn" class="success">บันทึกข้อมูลทั้งหมดลงไฟล์</button><button id="save-to-browser-btn" style="background-color: #007bff;">บันทึกข้อมูลปัจจุบันลงในเบราว์เซอร์</button></div><div class="data-management-section admin-only" style="border-color: var(--danger-color);"><h3 style="color: var(--danger-color);">รีเซ็ตข้อมูล (*** การกระทำนี้ไม่สามารถย้อนกลับได้ ***)</h3><p>เลือกเพื่อล้างข้อมูลเฉพาะส่วนที่ต้องการ</p><button id="open-reset-modal-btn" class="danger">เปิดหน้าต่างรีเซ็ตข้อมูล</button></div><div class="collapsible-bar admin-only" data-target="admin-stock-report-content" style="background-color: #00B050;"><span>รายงานสต็อกสินค้า</span><span class="arrow">▶</span></div><div id="admin-stock-report-content" class="collapsible-content admin-only"><div style="text-align:center; padding: 10px;"><p>รายงานนี้จะเปรียบเทียบสต็อกที่คำนวณได้จากประวัติ (นำเข้า - ขาย - ปรับออก) กับสต็อกที่บันทึกไว้ปัจจุบัน</p><button id="generate-stock-report-btn" class="success">สร้างรายงานสต็อก</button><button id="recalculate-stock-btn" class="danger">คำนวณสต็อกใหม่ทั้งหมด</button></div><div id="stock-summary-report-container" style="margin-top: 15px;"></div></div><div class="collapsible-bar seller-only" data-target="seller-backup-content"><span>บันทึกข้อมูล (Backup)</span><span class="arrow">▶</span></div><div id="seller-backup-content" class="collapsible-content seller-only"><div style="text-align:center; padding-top: 10px;"><p style="margin-top:0;">สำรองข้อมูลทั้งหมด (ผู้ใช้, สินค้า, ประวัติการขาย) ลงในไฟล์ JSON เพื่อเก็บไว้หรือย้ายไปยังเครื่องอื่น</p><button id="save-to-file-btn-seller" class="success">บันทึกข้อมูลทั้งหมดลงไฟล์</button><button id="save-to-browser-btn-seller" style="background-color: #007bff;">บันทึกข้อมูลปัจจุบันลงในเบราว์เซอร์</button></div></div><div class="collapsible-bar seller-only" data-target="seller-summary-content"><span>รายงานสรุป (สำหรับผู้ใช้ปัจจุบัน)</span><span class="arrow">▶</span></div><div id="seller-summary-content" class="collapsible-content seller-only"><div style="text-align:center;"><div style="display: flex; flex-wrap: wrap; gap: 10px; justify-content: center;"><button id="my-summary-today-btn" style="background-color: var(--warning-color);">สรุปยอดขายวันนี้</button><button id="my-summary-all-btn" style="background-color: #673ab7;">สรุปทั้งหมดของฉัน</button></div><div class="summary-form-inline" style="margin-top: 15px; justify-content: center; flex-direction: column; gap:10px; align-items: stretch;"><label>เลือกวันที่: <input type="date" id="my-summary-date" style="width:100%;"></label><button id="my-summary-by-day-btn" style="background-color: #03a9f4;">สรุปยอดขายวันที่เลือก</button></div></div></div><div class="collapsible-bar seller-only" data-target="seller-detailed-report-content" style="background-color: #ED01ED;"><span>แจกแจงรายละเอียดการขาย</span><span class="arrow">▶</span></div><div id="seller-detailed-report-content" class="collapsible-content seller-only"><form id="seller-detailed-report-form" class="summary-section" style="display: grid; grid-template-columns: 1fr; gap: 20px; max-width: 800px; margin: auto;"><div><h4 style="text-align: left; margin-bottom: 5px; padding-left: 10px;">1. เลือกประเภทการชำระ</h4><div id="seller-report-payment-types" style="display: flex; gap: 20px; flex-wrap: wrap; padding: 10px; background-color: #eef5ff; border-radius: 10px; justify-content: center;"><label style="font-weight: normal; cursor: pointer;"><input type="checkbox" value="เงินสด" checked> เงินสด</label><label style="font-weight: normal; cursor: pointer;"><input type="checkbox" value="เงินโอน" checked> เงินโอน</label><label style="font-weight: normal; cursor: pointer;"><input type="checkbox" value="เครดิต" checked> เครดิต</label></div></div><div><h4 style="text-align: left; margin-bottom: 5px; padding-left: 10px;">2. เลือกช่วงเวลา</h4><div class="summary-form-inline" style="justify-content: space-around;"><label style="font-weight: normal;">จากวันที่: <input type="date" id="seller-report-start-date" required></label><label style="font-weight: normal;">ถึงวันที่: <input type="date" id="seller-report-end-date" required></label></div></div><div class="form-actions"><button type="submit" class="success" style="width: 100%; max-width: 400px; padding: 15px; font-size: 1.2em;">3. สร้างรายงาน</button></div></form></div><div class="collapsible-bar seller-only" data-target="seller-credit-report-content" style="background-color: #d32f2f;"><span>สรุปข้อมูลลูกหนี้ (เครดิต)</span><span class="arrow">▶</span></div><div id="seller-credit-report-content" class="collapsible-content seller-only"><form id="seller-credit-report-form" class="summary-section" style="padding: 10px; margin: 0 auto; border: none;"><h4 style="text-align: center; margin-top:0;">เลือกช่วงเวลาที่ต้องการสรุป</h4><div class="summary-form-inline" style="justify-content: space-around;"><label style="font-weight: normal;">จากวันที่: <input type="date" id="seller-credit-start-date" required></label><label style="font-weight: normal;">ถึงวันที่: <input type="date" id="seller-credit-end-date" required></label></div><div class="form-actions" style="margin-top: 15px;"><button type="submit" class="danger" style="width: 100%; max-width: 400px;">สร้างรายงานลูกหนี้</button></div></form></div><div class="collapsible-bar seller-only" data-target="seller-transfer-report-content" style="background-color: #1976d2;"><span>สรุปข้อมูลเงินโอน</span><span class="arrow">▶</span></div><div id="seller-transfer-report-content" class="collapsible-content seller-only"><form id="seller-transfer-report-form" class="summary-section" style="padding: 10px; margin: 0 auto; border: none;"><h4 style="text-align: center; margin-top:0;">เลือกช่วงเวลาที่ต้องการสรุป</h4><div class="summary-form-inline" style="justify-content: space-around;"><label style="font-weight: normal;">จากวันที่: <input type="date" id="seller-transfer-start-date" required></label><label style="font-weight: normal;">ถึงวันที่: <input type="date" id="seller-transfer-end-date" required></label></div><div class="form-actions" style="margin-top: 15px;"><button type="submit" style="background-color: #007bff; width: 100%; max-width: 400px;">สร้างรายงานเงินโอน</button></div></form></div><div class="collapsible-bar seller-only active" data-target="seller-sales-history-container"><span>ค้นหารายการขาย</span><span class="arrow" style="transform: rotate(90deg);">▶</span></div><div id="seller-sales-history-container" class="collapsible-content seller-only active"><form id="seller-sales-filter-form" style="max-width: none; background-color: #eef5ff; padding: 15px; border-radius: 8px;"><div style="grid-column: 1/-1; display:flex; flex-wrap:wrap; gap: 20px; justify-content:center; align-items:center; margin-bottom: 10px;"><label><input type="radio" name="seller-filter-type" value="today" checked> วันนี้</label><label><input type="radio" name="seller-filter-type" value="by_date"> เลือกวัน</label><label><input type="radio" name="seller-filter-type" value="by_range"> เลือกช่วง</label></div><div id="seller-date-inputs" style="grid-column: 1/-1; display:flex; flex-wrap:wrap; gap: 15px; justify-content:center; align-items:flex-end;"><div id="seller-filter-by-date-div" style="display:none;"><label>เลือกวันที่:<input type="date" id="seller-filter-date"></label></div><div id="seller-filter-by-range-div" style="display:none; display:flex; gap:10px; flex-wrap:wrap; justify-content:center;"><label>จาก:<input type="date" id="seller-filter-start-date"></label><label>ถึง:<input type="date" id="seller-filter-end-date"></label></div></div><div class="form-actions" style="margin-top: 15px;"><button type="submit" style="background-color:#008CBA;">แสดงรายการ</button></div></form><div class="table-container" style="margin-top:20px;"><table id="seller-sales-history-table"><thead><tr><th>วันที่</th><th>เวลา</th><th>รายการสินค้า</th><th>ยอดขาย</th><th>ประเภทชำระ</th><th>จัดการ</th></tr></thead><tbody></tbody></table></div></div>`
            };
        ...
        attachEventListeners() {
            ...
            document.querySelectorAll('.section-header').forEach(header => {
                header.addEventListener('click', () => {
                    this.showPage(header.dataset.page);
                });
            });
            ...
            mainApp.addEventListener('click', (e) => {
                ...
                if (target.id === 'toggle-special-price-btn') this.toggleSpecialPrice();
                if (target.classList.contains('edit-sale-btn')) this.editSale(target.dataset.id);
                if (target.classList.contains('delete-sale-btn')) { this.deleteSale(target.dataset.id); this.renderSalesHistory(); }
                if (target.classList.contains('seller-delete-sale-btn')) {
                ...
                if (target.id === 'clear-product-form-btn') { document.getElementById('product-form').reset(); document.getElementById('product-id').value = ''; }
                if (target.classList.contains('edit-product-btn')) this.editProduct(target.dataset.id);
                if (target.classList.contains('delete-product-btn')) this.deleteProduct(target.dataset.id);
                ...
                if (target.classList.contains('delete-stock-in-btn')) this.deleteStockIn(target.dataset.id);
                if (target.id === 'clear-stock-in-form-btn') this.clearStockInForm();
                if (target.id === 'export-sales-history-csv-btn') this.exportSalesHistoryToCsv();
                
                const collapsibleBar = target.closest('.collapsible-bar');
                if (collapsibleBar) {
                    const contentId = collapsibleBar.dataset.target;
                ...
                if (target.id === 'load-from-file-btn') document.getElementById('data-file-input').click();
                if (target.id === 'save-to-file-btn' || target.id === 'save-to-file-btn-seller') this.saveBackupToFile();
                if (target.id === 'save-to-browser-btn' || target.id === 'save-to-browser-btn-seller') this.manualSaveToBrowser();
                if (target.id === 'open-reset-modal-btn') this.openResetModal();
                if (target.id === 'generate-stock-report-btn') this.renderStockSummaryReport();
                if (target.id === 'recalculate-stock-btn') this.handleRecalculateStock();
                if (target.id === 'my-summary-today-btn') this.summarizeMyToday();
                if (target.id === 'my-summary-all-btn') this.summarizeMyAll();
                if (target.id === 'my-summary-by-day-btn') this.summarizeMyDay();
                if (target.id === 'generate-detailed-report-btn') this.runAdminDetailedReport();
            ...
            document.body.addEventListener('change', (e) => {
                 const target = e.target;
                if (target.id === 'show-password-login') {
                    document.getElementById('password').type = target.checked ? 'text' : 'password';
                }
                if (target.id === 'show-password-user-form') {
                    document.getElementById('user-password').type = target.checked ? 'text' : 'password';
                    document.getElementById('user-password-confirm').type = target.checked ? 'text' : 'password';
                }
                if (target.id === 'show-backup-password') {
                    document.getElementById('backup-password').type = target.checked ? 'text' : 'password';
                ...
                if (target.id === 'reset-products-checkbox') {
                    if (target.checked) {
                        document.getElementById('reset-sales-checkbox').checked = true;
                        document.getElementById('reset-stockins-checkbox').checked = true;
                    }
                }
                if (target.id === 'pos-date' || target.id === 'pos-time') {
                    const dateInput = document.getElementById('pos-date');
                    const timeInput = document.getElementById('pos-time');
                    const isActive = dateInput.value || timeInput.value;
                    dateInput.classList.toggle('backdating-active', isActive);
                    timeInput.classList.toggle('backdating-active', isActive);
                }
                if (target.name === 'seller-filter-type') {
                    ...
                    byDateDiv.style.display = value === 'by_date' ? 'block' : 'none';
                    byRangeDiv.style.display = value === 'by_range' ? 'flex' : 'none';
                }
                if (target.id === 'stock-in-product') {
